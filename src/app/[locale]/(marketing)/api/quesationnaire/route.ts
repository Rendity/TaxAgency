...
export async function POST(req: NextRequest) {
  try {
    ...
    if (data.ibans.length > 0) {
      const ibanNodes = data.ibans.map(iban => ({
        name: maskIBAN(iban),
      }));
    ...
    if (data.creditCards.length > 0) {
      const cardNodes = data.creditCards.map(card => ({
        name: maskCard(card),
      }));
    ...
    return NextResponse.json({ success: true, created: parentFolder });
  } catch (err: any) {
